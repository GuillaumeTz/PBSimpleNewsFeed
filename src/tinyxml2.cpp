...
const char* XMLElement::AttributeIsEqual( const char* name, const char* value ) const
{
...
const char* XMLElement::GetAttribute(const char* name, const char* defaultValue) const
{
	const XMLAttribute* a = FindAttribute( name );
	if ( !a ) {
		return defaultValue;
...
int XMLElement::GetAttribute(const char* name, int defaultValue) const
{
	const XMLAttribute* a = FindAttribute( name );
	if ( !a ) {
		return defaultValue;
...
char* XMLElement::ParseAttributes( char* p )
{
	...
	while( p ) {
		...
		if (XMLUtil::IsNameStartChar( *p ) ) {
			...
			if ( !p || AttributeIsEqual( attrib->Name() ) ) {
